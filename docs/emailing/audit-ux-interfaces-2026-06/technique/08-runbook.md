...
|---|---|---|---|---|---|
| 2026-06-21 | P3.4-f (F07 Lot 2) — variablesResolvedMap (valeurs résolues au panneau) | session (worktree, feat/emails-ux-p0) | **F07-U-114/115** (resolvedVarsMap) + **F07-C-022** (panneau affiche les valeurs) ; templates 53/53 + resolved-vars 2/2 ; tsc RC=0 | Helper PUR en lib (pas d'import cross-layer route→components) ; tronque + exclut `trigger` (pas de dump/PII longue). | ✅ `resolvedVarsMap(context)` (lib) : carte clé→valeur affichable, coerce/tronque/exclut trigger ; route preview renvoie `variablesResolvedMap` ; le panneau de l'éditeur affiche en title « label → valeur » (ou « (vide) »). **Lot 2 assistance bouclé** (catalogue honnête + insertion curseur + autocomplete `{{` + valeurs résolues ; reste customVars Prettify, mineur). RESTE Lots 3-6 (preview mobile, **test-send sécurisé** prioritaire, CodeMirror, diff) + F07-S/F07-D + design G10. Commit à suivre. |
| 2026-06-21 | P3.4-e (F07 Lot 2) — dropdown autocomplete `{{` câblé | session (worktree, feat/emails-ux-p0) | **F07-C-018..021** (dropdown) = 4 ; templates 52/52 (7 fichiers) ; tsc RC=0 | Dropdown sous la textarea (pas de coordonnées caret — jsdom-testable) ; `onMouseDown` (pas click) pour agir AVANT le blur. Contrats préservés (la source n'est jamais saisie en `{{partiel` par les tests existants → 0 dropdown parasite). | ✅ Autocomplete `{{` END-TO-END : suit le curseur de la source (onChange/onClick/onKeyUp), liste `role=listbox`/`option` filtrée par le partiel ; clavier ↑/↓ + Entrée/Tab (accepte), Échap (ferme), clic ; `applyMergeCompletion` insère le jeton clos. RESTE Lot 2 : `variablesResolvedMap` (route preview → valeurs résolues dans le panneau) + customVars Prettify/clés manquantes ; puis Lots 3-6 (preview mobile, **test-send sécurisé**, CodeMirror, diff) + F07-S/F07-D + design G10. Commit à suivre. |
| 2026-06-21 | P3.4-d (F07 Lot 2) — cœur pur autocomplete `{{` | session (worktree, feat/emails-ux-p0) | **F07-U-108..113** (activeMergeQuery + applyMergeCompletion) = 6/6 ; tsc RC=0 | Cœur PUR avant le câblage du dropdown (pattern éprouvé) ; jsdom ne sait pas positionner un caret → on évite les coordonnées pixel, l'autocomplete s'appuiera sur ce parsing pur. | ✅ `merge-autocomplete.ts` : `activeMergeQuery(value,cursor)` détecte un `{{partiel` ouvert (null si fermé/espace/point → écarte la syntaxe Listmonk) ; `applyMergeCompletion` remplace le partiel par `{{cle}}` (consomme un `}}` déjà saisi → pas de `}}}}`), ou insère au curseur hors jeton. RESTE Lot 2 : **dropdown** d'autocomplete câblé (P3.4-e) + `variablesResolvedMap` (route preview) + customVars Prettify ; puis Lots 3-6 + F07-S/D. Commit à suivre. |
