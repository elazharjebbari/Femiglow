#!/usr/bin/env node
/**
 * Variante .mjs auto-exécutable (ne nécessite pas tsx) du lint
 * `check-event-emit-patterns.ts`. Utilisée par la CI quand tsx n'est pas
...
function checkFile(file, content) {
  ...
  const hasIdempotenceFlag = /useRef\([^)]*\)|sessionStorage|fired\.current/.test(content);
  const isTrackingPrimitive =
    /\/lib\/tracking\/(track|use-tracking|global-listener|client|provider)\.tsx?$/.test(file);
  ...
  for (let i = 0; i < lines.length; i++) {
      ...
      if (dynEmit) {
        ...
        if (varName !== 'eventName' && varName !== 'event') {
          issues.push({
            ...
            detail: `emit() avec nom non littéral "${varName}".`,
            excerpt: line.trim(),
          });
        }
      }
    ...
    if (
      ...
      issues.push({
        ...
        detail: 'emit("video_start"|"video_user_play") sans drapeau user-initiated.',
        excerpt: line.trim(),
      });
    }
  }
  ...
  while ((match = useEffectRegex.exec(content)) !== null) {
    ...
    if (/\bemit\(/.test(block)) {
      const hasGuard = /fired\.current|hasBeen|sessionStorage|once|return\s*;/.test(block);
      if (!hasGuard && !hasIdempotenceFlag) {
        issues.push({
          ...
          detail:
            'emit() dans un useEffect sans flag d\u2019idempotence détecté.',
