/**
 * GET /api/admin/analytics/insights/sections — top par durée moyenne.
 */
import { NextResponse } from 'next/server';
import { getAdminSession } from '@/lib/auth/require-admin';
import { formatErrorResponse, HttpError } from '@/lib/errors/http-error';
import { parseInsightsFiltersFromUrl } from '@/lib/analytics/insights/parse-filters';
import { getSectionsTop } from '@/lib/analytics/insights/services';
...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    const data = await getSectionsTop(filters, limit);
