...
import { importPostizStatusJob } from '@/lib/content-studio/automation';
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const result = await importPostizStatusJob({
      pastDays: numericParam(searchParams.get('pastDays'), 30),
      futureDays: numericParam(searchParams.get('futureDays'), 30),
      limit: numericParam(searchParams.get('limit'), 25),
