'use client';

import NextImage from 'next/image';
import { cn } from '@/lib/utils/cn';
import type { HeroGalleryImage } from '@/lib/products/hero-gallery-types';

export interface HeroGalleryThumbnailsProps {
  images: HeroGalleryImage[];
  currentIndex: number;
  onSelect: (index: number) => void;
  className?: string;
  /** `vertical` (desktop, colonne) ou `horizontal` (mobile, strip). Défaut vertical. */
  orientation?: 'vertical' | 'horizontal';
}

/**
 * Vignettes sélectionnables de la galerie. `vertical` = colonne desktop ;
 * `horizontal` = strip sous le carousel mobile (scroll horizontal, centré).
 */
export function HeroGalleryThumbnails({
  images,
  currentIndex,
  onSelect,
  className,
  orientation = 'vertical',
}: HeroGalleryThumbnailsProps): JSX.Element | null {
  if (images.length <= 1) return null;
  const horizontal = orientation === 'horizontal';

  return (
    <ul
      className={cn(
        horizontal
          ? 'flex max-w-full flex-row justify-center gap-1.5 overflow-x-auto px-3 pb-1 sm:gap-2 sm:px-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden'
          : 'flex max-h-[600px] flex-col gap-3 overflow-y-auto pe-1 [scrollbar-width:thin] [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-encre/15',
        className,
      )}
      aria-label="Vignettes produit"
    >
      {images.map((img, i) => {
        const active = i === currentIndex;
        return (
          <li key={img.id} className="shrink-0">
            <button
              type="button"
              aria-current={active ? 'true' : undefined}
              aria-label={`Voir l'image ${i + 1} sur ${images.length}${img.kind === 'review' ? ' (photo cliente)' : ''}`}
              onClick={() => onSelect(i)}
              className={cn(
                'group relative block aspect-[4/5] overflow-hidden rounded',
                horizontal ? 'w-[clamp(2rem,11vw,4rem)]' : 'w-16',
                'transition-all duration-200',
                active
                  ? 'ring-2 ring-[#4A5D4A] ring-offset-2 ring-offset-creme-warm'
                  : 'ring-1 ring-[#C7CCC2] hover:ring-[#4A5D4A]',
                'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4A5D4A] focus-visible:ring-offset-2',
                'active:scale-[0.97]',
              )}
            >
              {/* Vignette optimisée : Next/Image sert AVIF/WebP (négociation
                  + srcset) à la taille vignette (~64–128px) au lieu du JPEG brut.
                  cf. migration images 2026-06-01. */}
              <NextImage
                src={img.src}
                alt=""
                aria-hidden
                fill
                sizes="(min-width: 1024px) 128px, 80px"
                placeholder={img.blurDataURL ? 'blur' : 'empty'}
                blurDataURL={img.blurDataURL}
                className="object-cover"
              />
            </button>
          </li>
        );
      })}
    </ul>
  );
}
