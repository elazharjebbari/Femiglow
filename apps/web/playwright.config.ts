...
/**
 * Exclusion CI (2026-06-11) : les specs `ai-engine-*` (412 tests) sont
 * RÉPARÉS et verts contre staging depuis le 2026-06-11 (l'UI drift du 25/05
 * — HITL par défaut, locators ambigus, labels renommés — est résorbé), mais
 * restent exclus du job e2e BLOQUANT pour son runtime (workers=1 en CI →
 * >40 min). Ils tournent dans le job CI dédié non-bloquant `e2e-ai-engine`
 * (sans ce flag). `CS_E2E_SKIP_AI_ENGINE=true` ne sert qu'à scoper le job
 * bloquant ; la suite complète reste exécutable à la main (sans le flag).
 */
