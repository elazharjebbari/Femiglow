 ...
 * without layout breakage. Uses page.route() to mock all API calls.
 */
...
// ── Helper: set up all mocks for a given page ────────────────────
...
test('responsive — dashboard loads on mobile without horizontal overflow', async ({ page }) => {
  ...
  // Verify the AI Engine content area is not clipped — check the .cs-v2-shell
  // or main content does not use overflow:hidden that would hide content.
...
test('responsive — create form fields stack vertically on mobile', async ({ page }) => {
  ...
  // All 5 required field labels should be visible and not overlapping.
  ...
  // Verify fields are stacked: each successive field should have a greater Y
...
test('responsive — trends cards stack vertically on mobile', async ({ page }) => {
  ...
  // Card should be at least 80% of viewport width (375 * 0.8 = 300)
...
test('responsive — config provider cards stack on mobile', async ({ page }) => {
  ...
  const isCardVisible = await page.evaluate(() => {
    // Find the card containing "OpenAI"
...
test('responsive — knowledge collections are readable on mobile', async ({ page }) => {
  ...
  // Collection name should be visible
...
test('responsive — analytics KPI cards wrap properly on mobile', async ({ page }) => {
  ...
  // KPI cards should not overflow — verify they are all within the viewport
  // or scrollable area (not clipped)
  ...
  // Verify no horizontal overflow by checking content is not wider than scroll area
  const isNotOverflowing = await page.evaluate(() => {
    const shell = document.querySelector('.cs-v2-shell');
    if (!shell) return true;
    const main = shell.querySelector('main') ?? shell;
    return main.scrollWidth <= main.clientWidth + 50; // Allow small margin
  });
...
test('responsive — "Générer" button is fully visible and tappable on mobile', async ({ page }) => {
  ...
  // Fill required fields so button becomes enabled
  await page.locator('select').nth(0).selectOption('engagement');
  await page.locator('select').nth(1).selectOption('instagram');
  await page.locator('select').nth(2).selectOption('carousel');
  await page.locator('select').nth(3).selectOption('luxurious');
  ...
  expect(buttonBox!.width).toBeGreaterThan(40); // Not clipped too narrow
  expect(buttonBox!.height).toBeGreaterThan(20); // Not clipped too short

  // Tap the button (touch action) — should work on mobile
  await genButton.click();
  await expect(page.getByText('Pipeline de génération')).toBeVisible({ timeout: 15_000 });
});
